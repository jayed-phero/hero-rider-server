...
async function fetchAndSaveForCategories() {
  for (const categoryId of categoryIds) {
    ...
    // Fetch and save full Hadiths for each category
    const fullHadiths = await fetchAndSaveFullHadiths(categoryId, hadithIds);
...
// Fetch and save full Hadiths for a category
async function fetchAndSaveFullHadiths(categoryId, hadithIds) {
  const fullHadiths = [];

  for (const hadithId of hadithIds) {
    const fullHadith = await fetchFullHadith(hadithId);

    if (fullHadith) {
      fullHadiths.push(fullHadith);
    } else {
      console.error(`Error fetching Full Hadith for ID ${hadithId}: Skipped`);
    }
  }
